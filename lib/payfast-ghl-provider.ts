...
async function ghlRequest(path: string, token: string, method: 'POST' | 'PUT', body: unknown) {
  ...
  if (!response.ok) {
    throw new Error(`${response.status} ${typeof data === 'string' ? data : data?.message || data?.error || 'GHL request failed'}`);
  }
  return data;
}
...
async function ensureKeys(locationId: string) {
  const rows = await query<any[]>(
    'SELECT provider_api_key, provider_publishable_key FROM installations WHERE location_id=? LIMIT 1',
    [locationId],
  );
...
export async function provisionPayfastProvider(locationId: string, mode: 'live' | 'test') {
  ...
  try {
    const provider = await ghlRequest(
      `/payments/custom-provider/provider?${new URLSearchParams({ locationId })}`,
      token,
      'POST',
      {
        name: 'PayFast Connect by 10x Digital Ventures',
        description: 'Dedicated PayFast Pakistan payment provider',
        locationId,
        paymentsUrl: appUrl('/apps/payfast/checkout'),
        queryUrl: `${appUrl('/api/ghl/query-v2')}?locationId=${encodeURIComponent(locationId)}&app=payfast`,
        imageUrl: process.env.PAYFAST_GHL_PROVIDER_LOGO_URL || process.env.GHL_PROVIDER_LOGO_URL || appUrl('/logo.png'),
        supportsSubscriptionSchedule: true,
      },
    );
    const keys = await ensureKeys(locationId);
    const config = { apiKey: keys.apiKey, publishableKey: keys.publishableKey };
    const connection = await ghlRequest(
      `/payments/custom-provider/connect?${new URLSearchParams({ locationId })}`,
      token,
      'POST',
      { live: config, test: config, liveMode: mode === 'live' },
    );
    const capabilities = await ghlRequest(
