...
async function ensureKeys(locationId: string) {
  const rows = await query<any[]>(
    'SELECT provider_api_key, provider_publishable_key FROM payfast_ghl_installations WHERE location_id=? LIMIT 1',
    [locationId],
  );
  ...
  await query(
    'UPDATE payfast_ghl_installations SET provider_api_key=?, provider_publishable_key=? WHERE location_id=?',
  ...
  try {
    const provider = await ghlRequest(
      ...
      'POST',
      {
        ...
        imageUrl: process.env.PAYFAST_GHL_PROVIDER_LOGO_URL || appUrl('/logo.png'),
        supportsSubscriptionSchedule: true,
      },
    );
    const keys = await ensureKeys(locationId);
    const providerConfig = { apiKey: keys.apiKey, publishableKey: keys.publishableKey };
