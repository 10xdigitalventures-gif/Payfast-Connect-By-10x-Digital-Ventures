...
export async function POST(request: NextRequest) {
  ...
  if (!locationId || !ghlTransactionId) {
    return NextResponse.json(
      { error: "locationId and ghlTransactionId required" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (provRows.length && provRows[0].provider === "whop")
      paymentMethodLabel = "Whop";
  } catch (err) {
    console.warn(
      "[CRM Notify] provider lookup failed, defaulting to PayFast",
      err,
    );
  }
  ...
  if (
    ...
    const trialEnd = body.trialEndsAt
      ? Math.floor(new Date(body.trialEndsAt).getTime() / 1000)
      : 0;
    const periodEnd = body.periodEnd
      ? Math.floor(new Date(body.periodEnd).getTime() / 1000)
      : now + 30 * 24 * 3600;
    const subStatus =
      eventType === "subscription.trialing"
        ? "trialing"
